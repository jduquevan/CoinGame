...
class NaiveAgent(BaseAgent):
    def __init__(self,
        ...
        self.actor = VIPActor(in_size=self.in_size,
    ...
    def compute_reinforce_loss(self, log_probs_a, log_probs_b, states_a, rewards_a, rewards_b, hiddens_a, values_b, causal_rewards_b, is_cg, is_loaded=False):
        ...
        return pg_loss, positive_adv_ratio, torch.zeros(1).to(self.device)
...
class VIPAgent(BaseAgent):
    ...
    def compute_value_loss(self, values, targets, rewards, use_buffer=False, buffer_batch_size=128):
        values = torch.permute(torch.stack(values).reshape(-1, self.batch_size), (1, 0))[:, 0:-1]
        targets = torch.permute(torch.stack(targets).reshape(-1, self.batch_size), (1, 0))[:, 1:]
        rewards = torch.permute(torch.stack(rewards).reshape(self.rollout_len, -1), (1, 0))[:, 0:-1]
    ...
    def compute_reinforce_loss(self, 
        ...
        if is_loaded:
            ...
            for i in range(self.rollout_len-1):
                diced_returns_b.append(compute_loaded_dice(log_probs_a[:, i:],
                                                           ...
                                                           False,
                                                           self.max_len))
            diced_returns_b = torch.permute(torch.stack(diced_returns_b), (1, 0)) - values_b.detach()[:,:-1]
            diced_returns_b = (diced_returns_b - torch.mean(diced_returns_b))/torch.std(diced_returns_b)
        ...
        if is_loaded:
            diced_returns_a = compute_loaded_dice(log_probs_a, rev_gammas, advantages, rewards_a, self.device, True) - values_a[:, 0].detach()
            # diced_returns_a = (diced_returns_a - torch.mean(diced_returns_a))/torch.std(diced_returns_a)
            mask = 1*(advantages<0)*(diced_returns_b>0)
            pg_loss = torch.mean(diced_returns_a)
